...
export class ShoppingcartService {
  ...
  createItem(cardId: string, itemId: string) {//create item with productid
    return this.db.object('/shopping-card/' + cardId + '/items/' + itemId);
 ...
 async decreaseQaunataty(product){
    ...
    let  item$ = this.createItem(this.cardid, product.key);
     ...
     item$.snapshotChanges().pipe(take(1)).subscribe((item: any) => {
          ...
          if((item.payload.val().quantity)==-2){
